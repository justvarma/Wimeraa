  ...
  return <div className="space-y-4">
    ...
    {showForm && <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-4">
      ...
      <div className={sectionCls}><p className="font-bold text-slate-800 mb-2">1. Device Identity</p><div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        {["custId","plantId","deviceId","deviceName"].map(key => <input key={key} value={(form as any)[key]} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} placeholder={key} className={inputCls} />)}
      ...
      <div className={sectionCls}><p className="font-bold text-slate-800 mb-2">3. Availability Factor</p><div className="grid grid-cols-2 md:grid-cols-4 gap-2"><input value={form.availabilityPostTime} onChange={e => setForm(p => ({ ...p, availabilityPostTime: e.target.value }))} placeholder="post time" className={inputCls}/><input value={form.availabilityDutyCycle} onChange={e => setForm(p => ({ ...p, availabilityDutyCycle: e.target.value }))} placeholder="duty cycle" className={inputCls}/><input value={form.availabilityRunDuration} onChange={e => setForm(p => ({ ...p, availabilityRunDuration: e.target.value }))} placeholder="run duration" className={inputCls}/><select value={form.interlock} onChange={e => setForm(p => ({ ...p, interlock: e.target.value as any }))} className={selectCls}><option value="enable">enable</option><option value="disable">disable</option></select><select value={form.algorithm} onChange={e => setForm(p => ({ ...p, algorithm: e.target.value }))} className={selectCls}>{["0 — Undefined","1 — 1x Digital Pin Based","2 — 2x Digital Pin Based","3 — 3x Digital Pin Based","4 — CT with Threshold","5 — CT with Variation","255 — Cloud Based"].map(v => <option key={v} value={v.split(" ")[0]}>{v}</option>)}</select></div><div className="mt-2"><input value={depInputA} onChange={e => setDepInputA(e.target.value)} placeholder="dep value" className={inputCls}/><button onClick={() => { setForm(p => ({ ...p, availabilityDepValues: max10Push(p.availabilityDepValues, depInputA) })); setDepInputA("") }} className="ml-2 text-xs bg-slate-800 text-white px-2 py-1 rounded">Add</button></div></div>
      <div className={sectionCls}><p className="font-bold text-slate-800 mb-2">4. Performance Factor</p><div className="grid grid-cols-2 md:grid-cols-4 gap-2"><input value={form.performancePostTime} onChange={e => setForm(p => ({ ...p, performancePostTime: e.target.value }))} placeholder="post time" className={inputCls}/><input value={form.debounceTime} onChange={e => setForm(p => ({ ...p, debounceTime: e.target.value }))} placeholder="debounce time" className={inputCls}/><select value={form.partCountType} onChange={e => setForm(p => ({ ...p, partCountType: e.target.value as any }))} className={selectCls}><option value="digital">digital</option><option value="ai">ai</option><option value="other">other</option></select>{form.partCountType === "other" && <select value={form.otherPartCountType || ""} onChange={e => setForm(p => ({ ...p, otherPartCountType: e.target.value as any }))} className={selectCls}><option value="">OTH</option><option value="OTH1">OTH1</option><option value="OTH2">OTH2</option></select>}</div><div className="text-xs mt-2 text-slate-700">Part count pins: {pins.map(pin => <label key={pin} className="ml-2"><input type="checkbox" checked={form.partCountPins.includes(pin)} onChange={e => setForm(p => ({ ...p, partCountPins: e.target.checked ? [...p.partCountPins, pin] : p.partCountPins.filter(x => x !== pin) }))} /> {pin}</label>)}</div><div className="mt-2"><input value={depInputP} onChange={e => setDepInputP(e.target.value)} placeholder="dep value" className={inputCls}/><button onClick={() => { setForm(p => ({ ...p, performanceDepValues: max10Push(p.performanceDepValues, depInputP) })); setDepInputP("") }} className="ml-2 text-xs bg-slate-800 text-white px-2 py-1 rounded">Add</button></div></div>
