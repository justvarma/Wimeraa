...
function DevicesTab() {
  ...
  return <div className="space-y-4">
    ...
    {showForm && <div className="bg-white border rounded-xl p-4 space-y-4">
      <h3 className="font-black text-slate-900">{editingId ? "Edit Device" : "Add Device"}</h3>
      <div><p className="font-bold text-slate-800 mb-2">1. Device Identity</p><div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        {["custId","plantId","deviceId","deviceName"].map(key => <input key={key} value={(form as any)[key]} onChange={e => setForm(p => ({ ...p, [key]: e.target.value }))} placeholder={key} className="border rounded px-3 py-2 text-sm" />)}
      ...
      <div><p className="font-bold text-slate-800 mb-2">2. Gateway Identity</p><div className="grid grid-cols-2 md:grid-cols-4 gap-2"><input value={form.gatewayId} onChange={e => setForm(p => ({ ...p, gatewayId: e.target.value }))} placeholder="gatewayId" className="border rounded px-3 py-2 text-sm"/><input value={form.gatewayName} onChange={e => setForm(p => ({ ...p, gatewayName: e.target.value }))} placeholder="gatewayName" className="border rounded px-3 py-2 text-sm"/><select value={form.licensing} onChange={e => setForm(p => ({ ...p, licensing: e.target.value as any }))} className="border rounded px-3 py-2 text-sm bg-white"><option>1m</option><option>3m</option><option>6m</option><option>1yr</option></select><select value={form.gatewayType} onChange={e => setForm(p => ({ ...p, gatewayType: e.target.value as any, partCountPins: [] }))} className="border rounded px-3 py-2 text-sm bg-white"><option>Edj10</option><option>Edj20</option></select></div></div>
      <div><p className="font-bold text-slate-800 mb-2">3. Availability Factor</p><div className="grid grid-cols-2 md:grid-cols-4 gap-2"><input value={form.availabilityPostTime} onChange={e => setForm(p => ({ ...p, availabilityPostTime: e.target.value }))} placeholder="post time" className="border rounded px-3 py-2 text-sm"/><input value={form.availabilityDutyCycle} onChange={e => setForm(p => ({ ...p, availabilityDutyCycle: e.target.value }))} placeholder="duty cycle" className="border rounded px-3 py-2 text-sm"/><input value={form.availabilityRunDuration} onChange={e => setForm(p => ({ ...p, availabilityRunDuration: e.target.value }))} placeholder="run duration" className="border rounded px-3 py-2 text-sm"/><select value={form.interlock} onChange={e => setForm(p => ({ ...p, interlock: e.target.value as any }))} className="border rounded px-3 py-2 text-sm bg-white"><option value="enable">enable</option><option value="disable">disable</option></select><select value={form.algorithm} onChange={e => setForm(p => ({ ...p, algorithm: e.target.value }))} className="border rounded px-3 py-2 text-sm bg-white">{["0 — Undefined","1 — 1x Digital Pin Based","2 — 2x Digital Pin Based","3 — 3x Digital Pin Based","4 — CT with Threshold","5 — CT with Variation","255 — Cloud Based"].map(v => <option key={v} value={v.split(" ")[0]}>{v}</option>)}</select></div><div className="mt-2"><input value={depInputA} onChange={e => setDepInputA(e.target.value)} placeholder="dep value" className="border rounded px-2 py-1 text-sm"/><button onClick={() => { setForm(p => ({ ...p, availabilityDepValues: max10Push(p.availabilityDepValues, depInputA) })); setDepInputA("") }} className="ml-2 text-xs bg-slate-800 text-white px-2 py-1 rounded">Add</button></div></div>
      <div><p className="font-bold text-slate-800 mb-2">4. Performance Factor</p><div className="grid grid-cols-2 md:grid-cols-4 gap-2"><input value={form.performancePostTime} onChange={e => setForm(p => ({ ...p, performancePostTime: e.target.value }))} placeholder="post time" className="border rounded px-3 py-2 text-sm"/><input value={form.debounceTime} onChange={e => setForm(p => ({ ...p, debounceTime: e.target.value }))} placeholder="debounce time" className="border rounded px-3 py-2 text-sm"/><select value={form.partCountType} onChange={e => setForm(p => ({ ...p, partCountType: e.target.value as any }))} className="border rounded px-3 py-2 text-sm bg-white"><option value="digital">digital</option><option value="ai">ai</option><option value="other">other</option></select>{form.partCountType === "other" && <select value={form.otherPartCountType || ""} onChange={e => setForm(p => ({ ...p, otherPartCountType: e.target.value as any }))} className="border rounded px-3 py-2 text-sm bg-white"><option value="">OTH</option><option value="OTH1">OTH1</option><option value="OTH2">OTH2</option></select>}</div><div className="text-xs mt-2">Part count pins: {pins.map(pin => <label key={pin} className="ml-2"><input type="checkbox" checked={form.partCountPins.includes(pin)} onChange={e => setForm(p => ({ ...p, partCountPins: e.target.checked ? [...p.partCountPins, pin] : p.partCountPins.filter(x => x !== pin) }))} /> {pin}</label>)}</div><div className="mt-2"><input value={depInputP} onChange={e => setDepInputP(e.target.value)} placeholder="dep value" className="border rounded px-2 py-1 text-sm"/><button onClick={() => { setForm(p => ({ ...p, performanceDepValues: max10Push(p.performanceDepValues, depInputP) })); setDepInputP("") }} className="ml-2 text-xs bg-slate-800 text-white px-2 py-1 rounded">Add</button></div></div>
      <div><p className="font-bold text-slate-800 mb-2">5. In Pin Configuration</p><div className="grid grid-cols-2 gap-2"><input value={form.pinScanTime} onChange={e => setForm(p => ({ ...p, pinScanTime: e.target.value }))} placeholder="scan time" className="border rounded px-3 py-2 text-sm"/><input value={form.pinPostTime} onChange={e => setForm(p => ({ ...p, pinPostTime: e.target.value }))} placeholder="post time" className="border rounded px-3 py-2 text-sm"/></div></div>
      <div><p className="font-bold text-slate-800 mb-2">6. eMIC Configuration</p><div className="grid grid-cols-2 md:grid-cols-4 gap-2"><input value={form.emicPostTime} onChange={e => setForm(p => ({ ...p, emicPostTime: e.target.value }))} placeholder="post time" className="border rounded px-3 py-2 text-sm"/><select value={form.frequency} onChange={e => setForm(p => ({ ...p, frequency: e.target.value as any }))} className="border rounded px-3 py-2 text-sm bg-white"><option>50hz</option><option>60hz</option></select><select value={form.phaseSequence} onChange={e => setForm(p => ({ ...p, phaseSequence: e.target.value as any }))} className="border rounded px-3 py-2 text-sm bg-white"><option>1-1P2W</option><option>2-2P2W</option><option>3-3P3W</option><option>4-4P4W</option></select></div><div className="mt-2"><input value={depInputE} onChange={e => setDepInputE(e.target.value)} placeholder="config factor value" className="border rounded px-2 py-1 text-sm"/><button onClick={() => { setForm(p => ({ ...p, emicConfigValues: max10Push(p.emicConfigValues, depInputE) })); setDepInputE("") }} className="ml-2 text-xs bg-slate-800 text-white px-2 py-1 rounded">Add</button></div></div>
      <div className="flex gap-2"><button onClick={save} className="px-4 py-2 bg-blue-600 text-white rounded text-sm font-bold">{editingId ? "Update Device" : "Add Device"}</button><button onClick={() => setShowForm(false)} className="px-4 py-2 border rounded text-sm font-bold">Close</button></div>
    </div>}
